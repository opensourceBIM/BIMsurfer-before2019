...
function BimSurfer() {
 ...
 this.mouseCoordsWithinElement = function(event) {
  ...
  this.mouseUp = function(event) {
    if (!(othis.scene != null)) return;
    if (event.which === 1 && othis.viewport.mouse.leftDragDistance < othis.constants.mouse.pickDragThreshold && othis.viewport.mouse.middleDragDistance < othis.constants.mouse.pickDragThreshold) {
      if (othis.viewport.mouse.pickRecord != null) {
  ...
  this.mouseMove = function(event) {
    ...
    if(delta[0] == 0 || delta [1] == 0) return;  // avoids disappearing
													// object
  ...
  window.setView = function(view){
	  ...
	  if (othis.scene != null) {
		  ...
		  case (1):	// front
		      ...
		      lookAtNode.set('eye', {x: othis.propertyValues.viewfactor, y: 0, z: 0});
		  ...
		  case (2):	// side
			  othis.resetView();
	    	  lookAtNode = othis.scene.findNode('main-lookAt');
	    	  lookAtNode.set('eye', {x: 0, y: othis.propertyValues.viewfactor, z: 0});
	      	  lookAtNode.set('look', {x: 0, y: 0, z: 0});
	      	  othis.propertyValues.oldZoom = 10;
			  break;
		  case(3):	// top
	    	  ...
	    	  lookAtNode.set('eye', {x: 0, y: 0, z: othis.propertyValues.viewfactor});
	      	  lookAtNode.set('look', {x: 0, y: 0, z: 0});
	      	  othis.propertyValues.oldZoom = 10;
			  break;
		  }
  ...
  window.setTransparentLevel = function(factor)
  {
	...
	// if a wall element is selected, get the corresponding storey to set this
	// one transparent
  ...
  this.exposeSelectedStorey = function($parent, distance) {
    ...
    // find controls-tree-rel (jquery) elements and add them to ids array; they
    ...
    _ref = othis.scene.data().ifcTypes;  // ifc types; WallStandardCase,
											// Roof, Window, Stair
    for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {  // iterate all ifc
															// Types
      ...
      if (tagNode != null) {
        tagNode.eachNode((function() {  // for each node (scenejs) under the
          ...
          // create a new translate Node; hierarchies with this node attached
			// get translated
          translateNodeJson = { 
            	  type: 'translate',
      ...
      if (node != null) {
        // if selected element begins with dp_ (= special object)
  	  	if ($('#'+RegExp.escape(node.get("id"))).text().match(/^ dp_/))
  		{
      	  ...
      	  // insert a higlight node to highlight selected element; highlight
			// entire storey by adding a highlight node under parent (original
			// withount parent())
...
// class='snapshot-thumb'><a href='#' class='snapshot-delete'>x</a><img width='"
...
// "'></div></div>");
 ...
 /* end fix */
  ...
  this.registerDOMEvents = function() {
    ...
    othis.viewport.domElement.addEventListener('DOMMouseScroll', othis.mouseWheel, false);
  ...
  this.loadScene = function(scene) {
    ...
    try {
      ...
      othis.scene = SceneJS.scene('Scene');
      othis.viewportInit();
      if (othis.scene != null) {
